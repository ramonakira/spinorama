...
Then some specific functions from src.spinorama:

```{python}
from src.spinorama.load_spl_hv_txt import parse_graph_spl_hv_txt
from src.spinorama.load import filter_graphs
from src.spinorama.constant_paths import MEAN_MIN, MEAN_MAX, DEFAULT_FREQ_RANGE
```
...
#### Compute the PIR (Predicted In-Room Response)
```{python}
from src.spinorama.load import graph_melt, graph_unmelt
...
```{python}
plot_spin = plot.plot_spinorama(spin, plot.plot_params_default, None, False, DEFAULT_FREQ_RANGE)
...
```{python}
plot_spin = plot.plot_spinorama(spin, plot.plot_params_default, True, DEFAULT_FREQ_RANGE)
plot_spin.update_layout(my_layout)
plot_spin
```
